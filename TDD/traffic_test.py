...
class knownCarValues(unittest.TestCase):
    ...
    def test_get_on(self):
        for get_on_number, known_result in self.knownGetOnValues:
            testCar = vehicle.car("testCar", self.horsepower, self.max_speed, self.speed, self.rated_passenger_capacity,
                                  self.actual_passenger)
...
class knownTruckValues(unittest.TestCase):
    ...
    def test_loading(self):
        for additional_load, known_result in self.knownLoadingValues:
            testTruck = vehicle.truck("testTruck", self.horsepower, self.max_speed, self.speed, self.rated_load,
                                      self.actual_load)
            print("Test truck currently has %dkg of load and its max load is %dkg" % (
            testTruck.actual_load, testTruck.rated_load))
...
class truckBadInput(unittest.TestCase):
    def test_negative_load(self):
        ...
        self.assertRaises(vehicle.OutOfRangeError, vehicle.truck.get_actual_load,
                          vehicle.truck('test', 10, 10, 10, 5, -1))
...
class carBadInput(unittest.TestCase):
    def test_negative_passenger(self):
        ...
        self.assertRaises(vehicle.OutOfRangeError, vehicle.car.get_rated_passenger_capacity,
                          vehicle.car('test', 1, 10, 0, -1, 0))
        self.assertRaises(vehicle.OutOfRangeError, vehicle.car.get_actual_passenger,
                          vehicle.car('test', 1, 10, 0, 0, -1))
